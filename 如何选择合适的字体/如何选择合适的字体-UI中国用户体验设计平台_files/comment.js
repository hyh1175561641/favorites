	...
	$(document).on('click','.p-replay',function(){
		...
		if(!userid){
			...
			var html;
				html = '<form id="comment_inner'+comid+'" class="comment-form comment-form-inner pos" action="">';
				html += '<textarea id="textarea_inner'+comid+'" class="comment-area" name="" id=""></textarea>';
			...
			if($(this).parents(".comment-data").find(".comment-form-inner").length > 0){
					...
					$(this).parents(".item").siblings("li").find(".comment-form-inner").slideUp();
		    }
	...
	$(document).on('click','#rep-comment',function(){
		...
		$this.text('');
		$this.attr('disabled',true);
		var rContent = $this.parents("form").find("textarea").val();
        $.ajax({
			type:'post',
		    url:'/repComment',
...
$(function(){
	...
	if(conti == 1){
		$(".fixed-vote a").addClass("on").text("已投票");
	}
	$(".fixed-vote a").click(function(){
		$.ajax({
			url:"/Mycenter/Api/vote",
			type:"post",
			dataType:"json",
			data:{"projectid":pid},
			success:function(msg){
